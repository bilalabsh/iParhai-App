...
const SubjectsPage = () => {
  ...
  const toggleSeason = (uniqueSeasonKey) => {
    setExpandedSeason(uniqueSeasonKey === expandedSeason ? null : uniqueSeasonKey);
  };
  ...
  const handleNavigation = async (path, year, season, paperName) => {
    console.log(`QP button pressed for Year: ${year}, Season: ${season}, Paper: ${paperName}`);

    try {
      // Fetch data from the backend API
      const response = await axios.get(`http://localhost:5000/api/${season}/${paperName}/${year}/`);
    ...
    <Layout>
      <View style={styles.container}>
        ...
        <View style={styles.tabsContainer}>
          <TouchableOpacity onPress={() => setActiveTab("Topical")} style={[styles.tab, activeTab === "Topical" && styles.activeTab]}>
            <Text style={[styles.tabText, activeTab === "Topical" && styles.activeTabText]}>Topical</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => setActiveTab("Past Papers")} style={[styles.tab, activeTab === "Past Papers" && styles.activeTab]}>
            <Text style={[styles.tabText, activeTab === "Past Papers" && styles.activeTabText]}>Past Papers</Text>
          </TouchableOpacity>
        </View>
        {activeTab === "Topical" ? (
          ...
          <ScrollView style={styles.scrollView}>
            {paperData.map((paper, yearIndex) => (
              <View key={yearIndex} style={styles.subjectContainer}>
                <TouchableOpacity onPress={() => toggleYear(paper.year)} style={styles.subjectHeader}>
                  <Text style={styles.subjectTitle}>{paper.year}</Text>
                  <Text style={styles.icon}>{expandedYear === paper.year ? "−" : "+"}</Text>
                </TouchableOpacity>
                    ...
                    {paper.seasons.map((season, seasonIndex) => {
                      ...
                      return (
                        <View key={seasonIndex}>
                          <TouchableOpacity onPress={() => toggleSeason(uniqueSeasonKey)} style={styles.subjectHeader}>
                            <Text style={styles.subtopicText}>{season.season}</Text>
                            <Text style={styles.icon}>{expandedSeason === uniqueSeasonKey ? "−" : "+"}</Text>
                          </TouchableOpacity>
                          {expandedSeason === uniqueSeasonKey && (
                            <View style={styles.subtopicsContainer}>
                              {season.papers.map((paperName, paperIndex) => (
                                <View key={paperIndex} style={styles.paperContainer}>
                                  <Text style={styles.paperText}>{paperName}</Text>
                                  <TouchableOpacity
                                    style={styles.qpButton}
                                    onPress={() => handleNavigation(`/qp/${paperName}`, paper.year, season.season, paperName)}
                                  >
                                    <Text style={styles.buttonText}>QP</Text>
                                  </TouchableOpacity>
                                  <TouchableOpacity
                                    style={styles.msButton}
                                    onPress={() => handleNavigation(`/ms/${paperName}`, paper.year, season.season)}
                                  >
